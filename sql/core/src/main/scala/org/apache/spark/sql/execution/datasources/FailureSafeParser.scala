...
class FailureSafeParser[IN](
  ...
  private val corruptCauseFieldIndex = schema.getFieldIndex(columnNameOfCorruptRecordCause)
  ...
  private val toResultRow: (Option[InternalRow], () => UTF8String, () => UTF8String) => InternalRow = {
    if (corruptFieldIndex.isDefined) {
      (row, badRecord, badRecordException) => {
        ...
        if (corruptCauseFieldIndex.isDefined) {
          resultRow(corruptCauseFieldIndex.get) = badRecordException()
        }
        resultRow
      }
    } else {
  ...
  def parse(input: IN): Iterator[InternalRow] = {
    try {
      rawParser.apply(input).toIterator.map(row => toResultRow(Some(row), () => null, () => null))
    } catch {
      case e: BadRecordException => mode match {
        ...
        case FailFastMode =>
          throw new SparkException("Malformed records are detected in record parsing. " +
            s"Parse Mode: ${FailFastMode.name}. [" + e.record.toString() + "]", e.cause)
      }
    }
  }
}
